...
const AddFriendModal: React.FC<Props> = ({ show = false, setShow }) => {
  ...
  const onSubmit = methods.handleSubmit(async(input) => {
    ...
    const isValid = !!profile && !!input.address && validChains.includes(input.chain);
    ...
    if (isCrossChainRequest) {
      ...
      writeFn('sendCrossChainFriendRequest', [input.address, wormholeChainId, wormholeDestinationHub], isValid, crossChainFee as bigint);
    } else {
      ...
      <div className='px-10 h-fit py-7'>
        <FormProvider {...methods}>
          <form className='flex flex-col gap-y-7'>
            <Select name='chain' options={networks} defaultValue={networks.find(n => n.value == chainId)} />
            ...
            <Button
              ...
              disabled={!profile || !methods.getValues("address") || isSubmitting}
              onClick={onSubmit}
            >
