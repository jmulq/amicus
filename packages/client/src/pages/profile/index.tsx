...
const ProfilePage = () => {
  ...
  const acceptFriendRequest = async (req: { profile: `0x${string}`; chain: number }) => {
    ...
    if (isCrossChainRequest) {
      ...
      writeFn('acceptFriendRequest', [req.profile], !!connectedProfile && !!req.profile);
    }
  };
      ...
      <div className='flex gap-x-10'>
        <div className='flex flex-col gap-y-10 flex-1'>
          <div className='relative overflow-hidden h-[364px] border border-primary-300 rounded-lg'>
            ...
            />
            <div className='absolute bg-black font-bold py-3 bg-opacity-25 text-center bottom-0 right-0 left-0 text-white'>
          ...
          <div className='border border-primary-300 rounded-lg h-[30vh] px-4 py-3 bg-white'>
            ...
            <div className='py-3 space-y-3'>
              {profileInboundReqs.length ? (
                profileInboundReqs.map((req, index) => (
                  <div key={index} className='flex gap-x-2 justify-between items-center'>
                    <div className='flex items-center gap-x-2'>
                      <p>{getProfileName(req.profile)}</p>
                      <p className='text-gray-400 text-sm'>({getChainName(req.chain)})</p>
                    </div>
                    <div className='flex items-center gap-x-2'>
                      <Button
                        size='sm'
          ...
          <div className='border border-primary-300 rounded-lg h-[30vh] px-4 py-3 bg-white'>
            ...
            <div className='py-3 space-y-4'>
              {profileOutboundReqs.length ? (
                profileOutboundReqs.map((req, index) => (
                  <div key={index} className='flex justify-between gap-x-2 items-center'>
                    <div className='flex items-center gap-x-2'>
                      <p>{getProfileName(req.profile)}</p>
                      <p className='text-gray-400 text-sm'>({getChainName(req.chain)})</p>
                    </div>
